package goRest;

import goRest.Model.User;
import io.restassured.http.ContentType;
import io.restassured.response.Response;
import org.apache.commons.lang3.RandomStringUtils;
import org.testng.annotations.Test;

import java.util.HashMap;
import java.util.List;
import java.util.Locale;
import java.util.Map;

import static io.restassured.RestAssured.*;
import static org.hamcrest.Matchers.equalTo;

public class GoRestUsersTest {

    @Test(enabled = false)
    public void getUsers() {
        List<User> listUser =
                given()


                        .when()
                        .get("https://gorest.co.in/public/v1/users")


                        .then()
                        .statusCode(200)
                        .contentType(ContentType.JSON)
                        .log().body()
                        .extract().jsonPath().getList("data", User.class);

        for (User u : listUser) {
            System.out.println("u = " + u);
        }
    }
    // Daha önceki örneklerde Clas dönüşümleri için tüm yapıya karşılık gelen
    // gereken tüm classları yazarak dönüştürüp istediğimiz elemanlara ulaşıyorduk.
    // Burada ise aradaki bir veriyi clasa dönüştürerek bir list olarak almamıza
    // imkan veren JSONPATH i kullandık.Böylece tek class ise veri alınmış oldu
    // diğer class lara gerek kalmadan

    // path : class veya tip dönüşümüne imkan veremeyen direk veriyi verir. List<String> gibi
    // jsonPath : class dönüşümüne ve tip dönüşümüne izin vererek , veriyi istediğimiz formatta verir.


// Create User için POSTMAN de yapılanlar

//    JSON olarak gidecek body  {"name":"{{$randomFullName}}", "gender":"male", "email":"{{$randomEmail}}", "status":"active"}
//
//    header ın içine
//    Authorization  Bearer 36e95c8fd3e7eb89a65bad6edf4c0a62ddb758f9ed1e15bb98421fb0f1f3e57f
//
//    POST ile https://gorest.co.in/public/v1/users çağırdık
//    id yi okuduk ve global bir değişkene attık ki, diğer reqest larde kullanabilelim


    int userID = 0;

    @Test
    public void createUsers() {
        userID =
                given()
                        .header("Authorization", "Bearer 5abe71b142b74da1e50c9a0f5f457762168cea4b3d2e230d619ad6470a55c44b")
                        .contentType(ContentType.JSON)
                        .body("{\"name\":\"mesut\", \"gender\":\"male\", \"email\":\"" + getRandomEmail() + "\", \"status\":\"active\"}")


                        .when()
                        .post("https://gorest.co.in/public/v1/users")


                        .then()
                        .log().body()
                        .statusCode(201)
                        .extract().jsonPath().getInt("data.id");

        System.out.println("getRandomEmail() = " + getRandomEmail());
        System.out.println("userID = " + userID);
    }

    public String getRandomEmail() {
        String randomString = RandomStringUtils.randomAlphabetic(8).toLowerCase(Locale.ROOT);
        return randomString + "@gmail.com";
    }

    public void create() {
        userID =
                given()
                        .header("Authorization", "Bearer 5abe71b142b74da1e50c9a0f5f457762168cea4b3d2e230d619ad6470a55c44b")
                        .contentType(ContentType.JSON)
                        .body("{\"name\":\"mesut\", \"gender\":\"male\", \"email\":\"" + getRandomEmail() + "\", \"status\":\"active\"}")


                        .when()
                        .post("https://gorest.co.in/public/v1/users")


                        .then()
                        .log().body()
                        .statusCode(201)
                        .extract().jsonPath().getInt("data.id");

    }

    @Test(dependsOnMethods = "createUsers", priority = 1)
    public void getUserByID() {

        given()
                .pathParam("userID", userID)
                .log().uri()


                .when()
                .get("https://gorest.co.in/public/v1/users/{userID}")


                .then()
                .log().body()
                .statusCode(200)
                .body("data.id", equalTo(userID));

    }

    @Test(dependsOnMethods = "createUsers", priority = 2)
    public void updateUserByid() {
        String name = "Mesut Erbay";

        given()
                .pathParam("userID", userID)
                .header("Authorization", "Bearer 5abe71b142b74da1e50c9a0f5f457762168cea4b3d2e230d619ad6470a55c44b")
                .contentType(ContentType.JSON)
                .body("{\"name\":\"" + name + "\"}")


                .when()
                .put("https://gorest.co.in/public/v1/users/{userID}")


                .then()
                .log().body()
                .statusCode(200)
                .body("data.name", equalTo(name))
        ;


    }

    //(dependsOnMethods = "createUsers")
    @Test(priority = 3)
    public void deleteUserByid() {


        given()
                .pathParam("userID", userID)
                .header("Authorization", "Bearer 5abe71b142b74da1e50c9a0f5f457762168cea4b3d2e230d619ad6470a55c44b")
                .log().uri()


                .when()
                .delete("https://gorest.co.in/public/v1/users/{userID}")


                .then()
                .log().body()
                .statusCode(204)

        ;
    }


    @Test(priority = 4)
    public void deleteNegativeUserByid() {
        given()
                .pathParam("userID", userID)
                .header("Authorization", "Bearer 5abe71b142b74da1e50c9a0f5f457762168cea4b3d2e230d619ad6470a55c44b")

                .when()
                .delete("https://gorest.co.in/public/v1/users/{userID}")

                .then()
                .statusCode(404)

        ;


    }

    @Test
    public void responSample() {
        Response donenSonuc =
                given()//donen sonuclarin hepsi tek bir degiskene atildi

                        .when()
                        .get("https://gorest.co.in/public/v1/users")

                        .then()
                        .log().body()
                        .extract().response();

        List<User> userList= donenSonuc.jsonPath().getList("data",User.class);
        int total=donenSonuc.jsonPath().getInt("meta.pagination.total");
        int limit=donenSonuc.jsonPath().getInt("meta.pagination.limit");
        User firstUser=donenSonuc.jsonPath().getObject("data[0]",User.class);


        System.out.println("userList = " + userList);
        System.out.println("limit = " + limit);
        System.out.println("total = " + total);
        System.out.println("firstUser = " + firstUser);

        /*
        tek bir değişken almak için
        path veya jsonPath kullanılabilir

        tüm veriye ihtiyacın varsa .as(Genel.class)
        kullanılacak

        verinin içinden bir bölümü bir clasa atmak
        istersen jsonPath kullanılacak
         */
    }
    @Test
    public void createUsersBodyMAp() {
        Map<String,String> newUser=new HashMap<>(); // verileri istersek MAp olarak gonderebilirz. Content.Json onu jsona ceviriyorz.
        newUser.put("name","Mesut");
        newUser.put("gender","male");
        newUser.put("email",getRandomEmail());
        newUser.put("status","active");
        userID =
                given()
                        .header("Authorization", "Bearer 5abe71b142b74da1e50c9a0f5f457762168cea4b3d2e230d619ad6470a55c44b")
                        .contentType(ContentType.JSON)
                        .body( newUser)


                        .when()
                        .post("https://gorest.co.in/public/v1/users")


                        .then()
                        .log().body()
                        .statusCode(201)
                        .extract().jsonPath().getInt("data.id");

        System.out.println("getRandomEmail() = " + getRandomEmail());
        System.out.println("userID = " + userID);
    }

    @Test
    public void createUsersBodyObject() {

        User newUser=new User();
        newUser.setName("mesut");
        newUser.setGender("male");
        newUser.setEmail(getRandomEmail());
        newUser.setStatus("active");

        userID =
                given()
                        .header("Authorization", "Bearer 5abe71b142b74da1e50c9a0f5f457762168cea4b3d2e230d619ad6470a55c44b")
                        .contentType(ContentType.JSON)
                        .body( newUser)
                        .log().body()


                        .when()
                        .post("https://gorest.co.in/public/v1/users")


                        .then()
                        .log().body()
                        .statusCode(201)
                        .extract().jsonPath().getInt("data.id");


        System.out.println("userID = " + userID);
    }
}
